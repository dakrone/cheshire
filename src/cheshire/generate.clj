...
(defmacro fail [obj jg opts]
  `(try
     (to-json ~obj ~jg)
     (catch IllegalArgumentException _#
       (throw (or (get ~opts :ex)
                  (JsonGenerationException.
                   (str "Cannot JSON encode object of class: "
                        (class ~obj) ": " ~obj)))))))

(defmacro number-dispatch [jg obj opts]
  (let [g (tag (gensym 'jg))
        o (gensym 'obj)
...
(defn encode-seq
  ...
  (.writeStartArray jg)
  (doseq [i s]
    (generate jg i {:date-format *date-format*}))
  (.writeEndArray jg))
  ...
  (doseq [[k v] m]
    (.writeFieldName jg (if (instance? clojure.lang.Keyword k)
                          (if-let [ns (namespace k)]
                            ...
                            (name k))
                          (str k)))
    (generate jg v {:date-format *date-format*}))
  (.writeEndObject jg))
